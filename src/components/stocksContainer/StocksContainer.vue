  ...
  setup() {
    ...
    const init = async () => {
      if (route.params.type === 'stocks') {
        ...
        const _response = await api.getActivesList('RUS', 'obligations', 10, 1);
        response.value = _response.data;
        // console.log(response.value);
        items.value = [...response.value.ITEMS];
        createGrid();
      } else if (route.params.type === 'etf') {
        const _response = await api.getActivesList('RUS', 'shares_etf', 100, 1);
        response.value = _response.data;
        items.value = [...response.value.ITEMS];
        createGrid();
      } else if (route.params.type === 'indexes') {
        const _response = await api.getActivesList('RUS', 'indexes', 100, 1);
